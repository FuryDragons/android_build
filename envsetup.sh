...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/furydragons/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/furydragons/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/furydragons/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^furydragons_") ; then
        FURYDRAGONS_BUILD=$(echo -n $1 | sed -e 's/^furydragons_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $FURYDRAGONS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    elif (echo -n $1 | grep -q -e "^furydragons_") ; then
        # Fall back to furydragons_<product>
        FURYDRAGONS_BUILD=$(echo -n $1 | sed -e 's/^furydragons_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $FURYDRAGONS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        FURYDRAGONS_BUILD=
    fi
    export FURYDRAGONS_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${FURYDRAGONS_DEVICES_ONLY}" != "z" ]; then
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the furydragons_ github
        T=$(gettop)
        cd $T > /dev/null
        vendor/furydragons/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/furydragons/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/furydragons/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. $ANDROID_BUILD_TOP/vendor/furydragons/build/envsetup.sh
